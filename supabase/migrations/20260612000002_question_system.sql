...
-- 페어 상대방이 오늘 답변했는지 여부 확인용 (내용은 아님)
...
-- ─── RPC: 오늘의 질문 배정 (없으면 생성) ─────────────────────────────────────
-- SECURITY DEFINER로 실행해 INSERT 시 RLS를 우회한다
...
BEGIN
  ...
  -- 레벨/역할 제한 내에 남은 질문이 없으면 레벨 제한 없이 재시도
